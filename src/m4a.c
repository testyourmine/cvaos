...
EWRAM_DATA struct MP2KPlayerState gMPlayInfo_5 = {}; // 0x030052E0
EWRAM_DATA struct MP2KPlayerState gMPlayInfo_6 = {}; // 0x03005320
EWRAM_DATA struct MP2KPlayerState gMPlayInfo_0 = {}; // 0x03005360
EWRAM_DATA struct MP2KPlayerState gMPlayInfo_7 = {}; // 0x030053A0
EWRAM_DATA struct MP2KPlayerState gMPlayInfo_1 = {}; // 0x030053E0
EWRAM_DATA struct MP2KPlayerState gMPlayInfo_2 = {}; // 0x03005420
EWRAM_DATA struct MP2KPlayerState gMPlayInfo_4 = {}; // 0x03005460

EWRAM_DATA u8 gMPlayMemAccArea[0x10] = {}; // 0x030054A0

EWRAM_DATA struct MP2KPlayerState gMPlayInfo_3 = {}; // 0x030054B0

EWRAM_DATA struct SoundMixerState gSoundInfo = {};  // 0x030054F0
EWRAM_DATA void *gMPlayJumpTable[36] = {}; // 0x03005E70
EWRAM_DATA struct MixerSource gCgbChans[4] = {};  // 0x03005F00
...
void m4aSoundInit(void)
{
    ...
    CPU_COPY_32((void *)((s32)SoundMainRAM & ~1), SoundMainRAM_Buffer, sizeof(SoundMainRAM_Buffer));
...
void MPlayExtender(struct MixerSource *cgbChans)
{
    ...
    WRITE_16(REG_SOUNDCNT_X, SOUND_MASTER_ENABLE | SOUND_4_ON | SOUND_3_ON | SOUND_2_ON | SOUND_1_ON);
    WRITE_16(REG_SOUNDCNT_L, 0); // set master volume to zero
    WRITE_8(REG_NR12, 0x8);
    WRITE_8(REG_NR22, 0x8);
    WRITE_8(REG_NR42, 0x8);
    WRITE_8(REG_NR14, 0x80);
    WRITE_8(REG_NR24, 0x80);
    WRITE_8(REG_NR44, 0x80);
    WRITE_8(REG_NR30, 0);
    WRITE_8(REG_NR50, 0x77);
    ...
    CPU_FILL_32(0, cgbChans, sizeof(struct MixerSource) * 4);
...
void SoundInit(struct SoundMixerState *soundInfo)
{
    ...
    if (READ_32(REG_DMA1_CNT) & (DMA_REPEAT << 16))
        WRITE_32(REG_DMA1_CNT, ((DMA_ENABLE | DMA_START_NOW | DMA_32BIT | DMA_SRC_INC | DMA_DEST_FIXED) << 16) | 4);
    // if (READ_32(REG_DMA2_CNT) & (DMA_REPEAT << 16))
    //     WRITE_32(REG_DMA2_CNT, ((DMA_ENABLE | DMA_START_NOW | DMA_32BIT | DMA_SRC_INC | DMA_DEST_FIXED) << 16) | 4);
    WRITE_16(REG_DMA1_CNT_H, DMA_32BIT);
    // WRITE_16(REG_DMA2_CNT_H, DMA_32BIT);
    WRITE_16(REG_SOUNDCNT_X, SOUND_MASTER_ENABLE | SOUND_4_ON | SOUND_3_ON | SOUND_2_ON | SOUND_1_ON);
    // WRITE_16(REG_SOUNDCNT_H, SOUND_B_FIFO_RESET | SOUND_B_TIMER_0 | SOUND_B_LEFT_OUTPUT | SOUND_A_FIFO_RESET | SOUND_A_TIMER_0
    //     | SOUND_A_RIGHT_OUTPUT | SOUND_ALL_MIX_FULL);
    WRITE_16(REG_SOUNDCNT_H, SOUND_A_FIFO_RESET | SOUND_A_TIMER_0
        | SOUND_A_LEFT_OUTPUT | SOUND_A_RIGHT_OUTPUT | SOUND_ALL_MIX_FULL);
    WRITE_8(REG_SOUNDBIAS_H, (READ_8(REG_SOUNDBIAS_H) & 0x3F) | 0x40);
    WRITE_32(REG_DMA1_SRC, (s32)soundInfo->pcmBuffer);
    WRITE_32(REG_DMA1_DST, (s32)REG_FIFO_A);
    // WRITE_32(REG_DMA2_SRC, (s32)soundInfo->pcmBuffer + PCM_DMA_BUF_SIZE);
    // WRITE_32(REG_DMA2_DST, (s32)REG_FIFO_B);

    SOUND_INFO_PTR = soundInfo;
    CPU_FILL_32(0, soundInfo, sizeof(struct SoundMixerState));
    // CPU_FILL_32(0, soundInfo, 0x5000260 * 4);
...
void SampleFreqSet(u32 freq)
{
    ...
    WRITE_16(REG_TM0CNT_L, -(280896 / soundInfo->samplesPerFrame));
    ...
    WRITE_16(REG_TM0CNT_H, TIMER_ENABLE | TIMER_1CLK);
}
...
void m4aSoundMode(u32 mode)
{
    ...
    if (temp) {
        ...
        WRITE_8(REG_SOUNDBIAS_H, (READ_8(REG_SOUNDBIAS_H) & 0x3F) | temp);
    }
...
void SoundClear(void)
{
    ...
    while (i > 0) {
        chan->status = 0;
        i--;
        chan = (void *)((s32)chan + sizeof(struct MixerSource));
    }
...
void m4aSoundVSyncOff(void)
{
    ...
    if (soundInfo->lockStatus >= ID_NUMBER && soundInfo->lockStatus <= ID_NUMBER + 1) {
        ...
        CPU_FILL_32(0, soundInfo->pcmBuffer, sizeof(soundInfo->pcmBuffer));
    }
}
...
void m4aSoundVSyncOn(void)
{
    ...
    WRITE_16(REG_DMA1_CNT_H, DMA_ENABLE | DMA_START_SPECIAL | DMA_32BIT | DMA_REPEAT);
                ...
                switch (ch) {
                    case 1:
                        *nrx0ptr = channels->data.cgb.sweep;
                        // fallthrough
                    case 2:
                        *nrx1ptr = ((s32)channels->wav << 6) + channels->data.cgb.length;
                        goto init_env_step_time_dir;
                    case 3:
                        if (channels->wav != channels->current) {
                            *nrx0ptr = 0x40;
                            WRITE_32(REG_WAVE_RAM0, ((u32 *)channels->wav)[0]);
                            WRITE_32(REG_WAVE_RAM1, ((u32 *)channels->wav)[1]);
                            WRITE_32(REG_WAVE_RAM2, ((u32 *)channels->wav)[2]);
                            WRITE_32(REG_WAVE_RAM3, ((u32 *)channels->wav)[3]);
    ...
    envelope_complete:
        ...
        if (channels->data.cgb.cgbStatus & CGB_CHANNEL_MO_PIT) {
            if (ch < 4 && (channels->type & TONEDATA_TYPE_FIX)) {
                s32 dac_pwm_rate = READ_8(REG_SOUNDBIAS_H);
        ...
        if (channels->data.cgb.cgbStatus & CGB_CHANNEL_MO_VOL) {
            WRITE_8(REG_NR51, (READ_8(REG_NR51) & ~channels->data.cgb.panMask) | channels->data.cgb.pan);
            if (ch == 3) {
